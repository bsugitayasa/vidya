...
import EditUraianAnggaran from './EditUraianAnggaran';

const inputClass = 'w-full rounded-xl border border-slate-200 p-3 text-sm outline-none focus:border-emerald-500';
const today = () => new Date().toISOString().slice(0, 10);
const verificationAvailableFor = (document, type) => {
  const contextualFlag = type === 'LPJ' ? document.tersediaUntukLpj : document.tersediaUntukRab;
  if (type === 'LPJ') {
    return contextualFlag ?? Boolean(document.namaPejabat && document.jabatan && document.namaPejabat2 && document.jabatan2);
  }
  return contextualFlag ?? document.tersedia;
};
...
export default function RabDetail() {
  ...
  const archiveId = searchParams.get('arsip');
  const isArchive = !!archiveId;
  const { user } = useAuthStore();
  const treasurer = !isArchive && ['BENDAHARA', 'SUPER_ADMIN'].includes(user?.role);
  ...
  const load = async () => {
    try {
      ...
      const displayed = archiveId ? await api.get(`/keuangan/rab/${id}/arsip/${archiveId}`) : rabRes;
      setRab({ ...displayed.data.data, audits: displayed.data.data.audits || [] }); setCategories(catRes.data.data.filter((r)=>r.isAktif)); setAccounts(accountRes.data.data.filter((r)=>r.isAktif)); setPrograms((programRes.data.data||programRes.data||[]).filter((r)=>r.isAktif!==false)); setVerificationDocuments(verificationRes.data.data||[]);
    } catch (error) { toast.error(error.response?.data?.message || 'Gagal memuat detail RAB'); }
  };
  useEffect(() => { setRab(null); load(); }, [id, archiveId]);
  ...
  const downloadExcel = async () => {
    try { const res = await api.get(`/keuangan/rab/${id}/export.xlsx`, { responseType: 'blob', params: archiveId ? { arsip: archiveId } : {} }); const url = URL.createObjectURL(res.data); const link = document.createElement('a'); link.href=url; link.download=`LPJ-${rab.nomorRab.replaceAll('/','-')}.xlsx`; link.click(); URL.revokeObjectURL(url); } catch { toast.error('Gagal mengunduh Excel'); }
  };
  ...
  const exportPdf = async () => {
    ...
    autoTable(doc,{startY:y,margin:{top:38,bottom:20},theme:'plain',body:[['Revisi',rab.revision],['Pencairan Bendahara',formatRupiah(rab.ringkasan.pencairanBendahara)],['Hibah / Punia / Lainnya',formatRupiah(rab.ringkasan.danaTambahan)],['Penerimaan Menunggu Verifikasi',formatRupiah(rab.ringkasan.penerimaanMenunggu)]],styles:{fontSize:8}});
    ...
    const disclaimer=verification?'Dokumen ini ditandatangani secara elektronik menggunakan verifikasi QR-Code terenkripsi sistem. Informasi di atas adalah mutlak benar dan sesuai dengan data resmi terverifikasi secara elektronik di dalam server PDPN.':'Dokumen ini belum disetujui dan belum ditandatangani secara elektronik.';
  ...
  const canSpend = !isArchive && ['DICAIRKAN_SEBAGIAN','DICAIRKAN_PENUH','REALISASI','PERLU_REVISI','DALAM_PENYESUAIAN'].includes(rab.status);
  ...
  const approveRabAction = () => {
    ...
    action(`/keuangan/rab/${id}/${rab.status === 'DIAJUKAN' ? 'approve' : 'sign-rab'}`, {
      ...form,
  ...
  return <div className="space-y-6">
    ...
    <div className="rounded-2xl border border-slate-100 bg-white p-5 shadow-sm"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="font-bold text-slate-800">Alur Persetujuan</h2><p className="text-xs text-slate-500">Aksi yang tersedia menyesuaikan status dan role Anda.</p></div><div className="flex flex-wrap gap-2">
      ...
      {treasurer&&canSpend&&rab.ringkasan.sisaKas>0&&<button onClick={()=>open('pengembalian',{tanggal:today(),akunKasId:accounts[0]?.id||''})} className="rounded-lg bg-amber-500 px-3 py-2 text-sm font-bold text-white">Pengembalian</button>}
      {!isArchive&&['REALISASI','DICAIRKAN_PENUH','DICAIRKAN_SEBAGIAN','PERLU_REVISI','DALAM_PENYESUAIAN'].includes(rab.status)&&<button onClick={()=>action(`/keuangan/rab/${id}/submit-lpj`)} className="rounded-lg bg-slate-800 px-3 py-2 text-sm font-bold text-white">Ajukan LPJ</button>}
