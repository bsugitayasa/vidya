...
export default function RabList() {
  ...
  const [form, setForm] = useState({ namaKegiatan: '', programAjahanId: '', penanggungJawab: '', tujuan: '', tanggalMulai: '', tanggalSelesai: '', catatan: '', dokumen: null, items: [emptyItem()] });
  const showForm = params.get('new') === '1';

  const load = async () => {
    setLoading(true);
    try {
      const [rabRes, categoryRes, programRes] = await Promise.all([
        api.get('/keuangan/rab', { params: { search: search || undefined, status: status || undefined, limit: 100 } }),
        api.get('/keuangan/kategori'), api.get('/program-ajahan')
      ]);
  ...
  const closeForm = () => { setParams({}); setForm({ namaKegiatan: '', programAjahanId: '', penanggungJawab: '', tujuan: '', tanggalMulai: '', tanggalSelesai: '', catatan: '', dokumen: null, items: [emptyItem()] }); };
  ...
  return <div className="space-y-6">
    ...
    {showForm && <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-slate-950/60 p-4 md:p-8"><form onSubmit={submit} className="w-full max-w-5xl overflow-hidden rounded-3xl bg-white shadow-2xl"><div className="flex items-center justify-between border-b border-slate-100 p-6"><div><h2 className="text-xl font-black text-slate-800">Buat Draft RAB</h2><p className="text-sm text-slate-500">Nomor RAB dibuat otomatis oleh sistem.</p></div><button type="button" onClick={closeForm} className="rounded-full p-2 hover:bg-slate-100"><X /></button></div><div className="space-y-6 p-6">
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2"><label className="space-y-1 text-sm font-semibold text-slate-600">Nama Kegiatan<input required value={form.namaKegiatan} onChange={(e) => setForm({...form, namaKegiatan:e.target.value})} className="w-full rounded-xl border border-slate-200 p-3 font-normal" /></label><label className="space-y-1 text-sm font-semibold text-slate-600">Program Ajahan<select value={form.programAjahanId} onChange={(e) => setForm({...form, programAjahanId:e.target.value})} className="w-full rounded-xl border border-slate-200 p-3 font-normal"><option value="">Kegiatan umum</option>{programs.map((program) => <option key={program.id} value={program.id}>{program.nama}</option>)}</select></label><label className="space-y-1 text-sm font-semibold text-slate-600">Penanggung Jawab<input required value={form.penanggungJawab} onChange={(e) => setForm({...form, penanggungJawab:e.target.value})} className="w-full rounded-xl border border-slate-200 p-3 font-normal" /></label><label className="space-y-1 text-sm font-semibold text-slate-600">Dokumen Pendukung (opsional)<input type="file" accept=".pdf,.jpg,.jpeg,.png" onChange={(e) => setForm({...form, dokumen:e.target.files[0]})} className="w-full rounded-xl border border-slate-200 p-2 text-sm font-normal" /></label><label className="space-y-1 text-sm font-semibold text-slate-600">Tanggal Mulai<input required type="date" value={form.tanggalMulai} onChange={(e) => setForm({...form, tanggalMulai:e.target.value})} className="w-full rounded-xl border border-slate-200 p-3 font-normal" /></label><label className="space-y-1 text-sm font-semibold text-slate-600">Tanggal Selesai<input required type="date" min={form.tanggalMulai} value={form.tanggalSelesai} onChange={(e) => setForm({...form, tanggalSelesai:e.target.value})} className="w-full rounded-xl border border-slate-200 p-3 font-normal" /></label><label className="space-y-1 text-sm font-semibold text-slate-600 md:col-span-2">Tujuan Kegiatan<textarea value={form.tujuan} onChange={(e) => setForm({...form, tujuan:e.target.value})} className="w-full rounded-xl border border-slate-200 p-3 font-normal" rows="2" /></label></div>
