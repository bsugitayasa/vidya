...
export default function RekonsiliasiKeuangan() {
  ...
  const pdf = async () => {
    ...
    autoTable(doc,{rowPageBreak:'avoid',startY:43,margin:{top:43,bottom:24,left:10,right:10},head:[['RAB','Kegiatan / Status',...columns.map(([,label])=>label)]],body,styles:{fontSize:6.5,cellPadding:1.5,overflow:'linebreak',halign:'right'},columnStyles:{0:{cellWidth:28,halign:'left'},1:{cellWidth:45,halign:'left'}},headStyles:{fillColor:[20,83,45]},didDrawPage:()=>{
      ...
      doc.setFontSize(14);doc.setTextColor(255);doc.setFont('helvetica','bold');doc.text('REKONSILIASI DANA KELOLAAN RAB',36,11);doc.setFontSize(8);doc.setFont('helvetica','normal');doc.text(period,36,18);
      doc.setFont('helvetica','italic');doc.setFontSize(5.5);doc.text('SK KEMENAG RI DIRJEN BIMAS HINDU NO. 471/DJ.VI/BA.01.1/03/2026',36,23);doc.text('SK Kemenkumham RI No. AHU-0000052.AH.01.07.Tahun 2020',36,27);
