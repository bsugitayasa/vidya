...
const mataKuliahSchema = z.object({
  ...
  sks: z.number().int().positive('SKS harus angka positif'),
  semester: z.number().int().positive('Semester harus angka positif'),
  programAjahanId: z.number().int().positive('Pilih Program Ajahan')
});

const createSesiSchema = z.object({
  mataKuliahId: z.number().int().positive('ID Mata Kuliah tidak valid'),
  tanggal: z.string().or(z.date()),
  pertemuan: z.number().int().positive('Nomor pertemuan harus positif'),
